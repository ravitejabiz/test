...
s3_file_name_main = "Process/AP/AP/interface_ap_sp.dat"
...
pandas_df = filtered_dataframe.toPandas()#.encode(encoding)
print(pandas_df)
# df_pandas_encoded = pandas_df.encode(encoding)
# print(df_pandas_encoded)
# spark_df = spark.createDataFrame(df_pandas_str)
# spark_df.show()


# Apply encoding to string values in the Pandas DataFrame
encoding = 'utf-8'
df_pandas = pandas_df.applymap(lambda x: x.encode(encoding).decode(encoding) if isinstance(x, str) else x)
